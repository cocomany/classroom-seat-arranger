...
> 使用智能座位编排技能，根据这份学生名单生成三套座位方案。近视严重的尽量坐前两排，爱讲话的学生分开，参考上次座位兼顾公平性，最后导出校园主题图片。

智能体会读取 [SKILL.md](SKILL.md)，按需调用确定性脚本完成导入、求解和渲染。图片或 PDF 名单由宿主智能体的视觉或文档能力先识别，再进入相同流程。

## 命令行

脚本只依赖 Python 标准库，建议使用 Python 3.10 及以上版本。导出 PNG 时需要本机安装 Chrome、Chromium、Edge 或其他兼容浏览器。
...
自检覆盖名单导入、确定性排座、硬规则、公平轮换、HTML 渲染，以及可用时的 PNG 导出。
...
作者：越山（GitHub [@cocomany](https://github.com/cocomany)）。商店页面显示的发布者身份以实际登录的 SkillHub 账号或发布者资料为准。

## 隐私

学生姓名、成绩、视力、课堂表现和职务属于敏感信息。技能默认支持本地处理；请只保留排座所需字段，不要把真实学生数据、生成历史或 `.env` 提交到仓库。
